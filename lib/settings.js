...
function readSettings() {
  ...
  return {
    heliusApiKey: fromConfigOrEnv(config, "heliusCredential", "HELIUS_API_KEY"),
    solanaRpcUrl,
    cheshireApiKey: fromConfigOrEnv(config, "cheshireCredential", "CHESHIRE_API_KEY"),
    xaiApiKey: fromConfigOrEnv(config, "xaiCredential", "XAI_API_KEY"),
    ...
    meshUrl:
      fromConfigOrEnv(config, "meshUrl", "CLAWD_MESH_URL") || "https://mesh.x402.wtf",
    meshModel:
      fromConfigOrEnv(config, "meshModel", "CLAWD_MESH_MODEL") || "qwen2.5:1.5b",
  };
}
...
function envVars(settings) {
  return {
    ...
    CLAWD_MESH_URL: settings.meshUrl,
    CLAWD_MESH_MODEL: settings.meshModel,
  };
}
