...
function meshCatalogEntry(extensionPath, settings) {
  return {
    id: "clawd-mesh",
    label: "Clawd Mesh (free)",
    kind: "stdio",
    ready: true,
    reason: "",
    plugin: "clawd",
    spec: {
      ...
      args: [path.join(extensionPath, "mcp", "clawd-mesh.js")],
      env: {
...
function catalogFromPlugins(extensionPath, settings) {
  ...
  const seen = new Set();
  const entries = [meshCatalogEntry(extensionPath, settings)];
  seen.add(identity("stdio", entries[0].spec));
